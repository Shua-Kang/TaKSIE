...
def evaluate_policy(model, env, epoch, eval_log_dir=None, debug=False, create_plan_tsne=False, running_config=""):
    """
    ...
    video_save_path = os.path.join(eval_log_dir, "saved_videos")
    os.makedirs(video_save_path, exist_ok=True)
    for id, (initial_state, eval_sequence) in enumerate(eval_sequences):

        model.reset_videos()
        result = evaluate_sequence(
            env, model, task_oracle, initial_state, eval_sequence, val_annotations, plans, debug)
        model.save_videos(os.path.join(video_save_path,
                                       f"{id}_{str(eval_sequence)}.mp4"))
        results.append(result)

        if not debug:
            eval_sequences.set_description(
                " ".join([f"{i + 1}/5 : {v * 100:.1f}% |" for i,
                         v in enumerate(count_success(results))]) + "|"
            )
        print(results)
...
def evaluate_sequence(env, model, task_checker, initial_state, eval_sequence, val_annotations, plans, debug):
    ...
    for subtask in eval_sequence:
        success = rollout(env, model, task_checker, subtask,
...
def main():
    ...
    if args.custom_model:
        ...
        if args.checkpoints is None and args.last_k_checkpoints is None and args.checkpoint is None:
            ...
            print(
                f"Evaluating model with last {args.last_k_checkpoints} checkpoints.")
            checkpoints = get_all_checkpoints(
                Path(args.train_folder))[-args.last_k_checkpoints:]
        ...
        for checkpoint in checkpoints:
            ...
            evaluate_policy(model, env, epoch, eval_log_dir=args.eval_log_dir,
                            debug=args.debug, create_plan_tsne=True)
