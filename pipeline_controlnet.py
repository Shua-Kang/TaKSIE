...
class StableDiffusionControlNetPipelineLstm(DiffusionPipeline, TextualInversionLoaderMixin):
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance or True:
    ...
    def prepare_image(
        ...
        if do_classifier_free_guidance and not guess_mode:
            image = torch.cat([image, torch.zeros_like(image), image])
    ...
    def __call__(
        ...
        class_label = None,
        save_diffusion_process_file_name = None
    ):
        r"""
        ...
        if isinstance(controlnet, ControlNetModelLstm):
            ...
            assert False
        self.if_normalize_conditioning = True
        if(self.if_normalize_conditioning):
            image = image * 2 - 1
            # print(image.min())
            # print(image.max())
        # 5. Prepare timesteps
        ...
        for i, t in enumerate(timesteps):
            ...
            latent_model_input = torch.cat([latents,  latents, latents]) if do_classifier_free_guidance else latents
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred_uncond,noise_pred_image, noise_pred = noise_pred.chunk(3)
            # print(noise_pred.shape)
            # print(t.shape)
            # print(latents.shape)
            ...
            if(save_diffusion_process_file_name is not None):
                image_for_dp = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
                do_denormalize = [True] * image.shape[0]
                image_for_dp = self.image_processor.postprocess(image_for_dp, output_type=output_type, do_denormalize=do_denormalize)
                image_for_dp[0].save(save_diffusion_process_file_name + "_" + str(t.item()) + ".jpg")
